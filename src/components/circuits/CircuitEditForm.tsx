...
import { updateCircuit, getLocations } from '../../lib/api';
...
export function CircuitEditForm({ circuit, onClose }: CircuitEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    monthlycost: circuit.monthlycost,
    static_ips: circuit.static_ips,
    upload_bandwidth: circuit.upload_bandwidth || '',
    contract_start_date: circuit.contract_start_date || new Date().toISOString().split('T')[0],
    contract_term: circuit.contract_term || 12,
    contract_end_date: circuit.contract_end_date || '',
    billing: circuit.billing,
    usage_charges: circuit.usage_charges,
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () => updateCircuit(circuit.id, formData),
    onSuccess: () => {
